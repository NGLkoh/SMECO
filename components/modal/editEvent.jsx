...
const EditEvent =  ({modalEditEvent, closeEditEventModal, refresh, editState}) => {
   ...
   const handleEditEvent = async () => {
		    ...
		    await axios.post('/api/event/updateById', { id: editState._id,  data: editorState})
            refresh()
            closeEditEventModal()
   }
  ...
  return (
    <>
      <Modal isOpen={modalEditEvent} onClose={() => closeEditEventModal()}>
        <ModalOverlay />
        <ModalContent>
          ...
          <ModalBody>
            <Text fontSize={11} mb={2}>Event Image:</Text>
             <FileUploader handleChange={(e) => handleChange(e)} name="file" types={fileTypes} />
               ...
               <Text fontSize={11} mt={2} mb={2}>Decription:</Text>
